...
class PictureGenerator:
    ...
    def generate_pictures(
        ...
        for base_path, _, files in walk(source_dir):
            ...
            print(f"Generate {nb_images_to_generate} for {base_path}")
            for filename in files:
                if not nb_iterations_per_image:
                    return

                self._generate_pictures_from_file(
    ...
    def _generate_pictures_from_file(
        ...
        for index, batch in enumerate(self._get_pictures(filename, base_path, nb_pictures_out)):
            ...
            print(path.join(destination_dir, f"generated_{index:02d}_{filename}"))

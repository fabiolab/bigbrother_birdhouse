...
class DataProcessor:
    ...
    def gererate_augmented_dataset(self, pictures_source_dir: str, pictures_destination_dir: str,
        ...
        for root, _, filenames in os.walk(pictures_source_dir):
            ...
            logger.info('')
            logger.info('nb pictures wished : {}, nb pictures found : {}, nb pictures to generate : {}'.format(
            ...
            for file in filenames:
                ...
                for batch in image_generator.flow(x, batch_size=1):
                    ...
                    print("\rnb pitures to generates {}".format(nb_images_to_generate), end='')
    ...
    def build_pictures_set(self):
        ...
        classes = [each for each in contents if os.path.isdir(os.path.join(self.collect_dir, each))]
    ...
    def save_bottlenecks(self, class_indices_path: str):
        ...
        np.save(os.path.join(self.root_dir, 'bottleneck_features_train.npy'), bottleneck_features_train)
        logger.info("done.")
        ...
        np.save(os.path.join(self.root_dir, 'bottleneck_labels_train.npy'), train_labels)
        logger.info("done.")
        ...
        np.save(os.path.join(self.root_dir, 'bottleneck_features_validation.npy'), bottleneck_features_validation)
        logger.info("done.")
        ...
        np.save(os.path.join(self.root_dir, 'bottleneck_labels_validation.npy'), validation_labels)
        logger.info("done.")
        ...
        if self.with_test_set:
            ...
            bottleneck_features_test = model.predict_generator(generator, predict_size_test, verbose=1)

            logger.info("saving bottleneck_features_test.npy")
            np.save(os.path.join(self.root_dir, 'bottleneck_features_test.npy'), bottleneck_features_test)
            logger.info("done.")
            ...
            np.save(os.path.join(self.root_dir, 'bottleneck_labels_test.npy'), test_labels)
            logger.info("done.")
    ...
    def __init__(self, root: str, with_test_set: bool = False, start_from_scratch: bool = True):
        configure('INFO')
        ...
        if os.path.isdir(self.collect_dir):
            ...
            logger.info('Found {} classes in collect directory : {} '.format(len(self.list_classes), self.list_classes))
